...
mkfs.ext4 -O ^64bit -L NeuroGentoo "${OPENSTACK_IMG_LODEV}p1"

OPENSTACK_IMG_UUID="$(dumpe2fs ${OPENSTACK_IMG_LODEV}p1 | sed -n 's/.*UUID:[[:space:]]\+\([a-z0-9\-]\+\).*/\1/p')"
